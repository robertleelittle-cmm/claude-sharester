...
- `src/sync.js` — Orchestrates a sync run: dispatches each source to its adapter by `source.type`, prunes stale command symlinks, promotes skill files out of `commands/` (via `demoteCommandLinks`), writes new command symlinks and skill dirs, then prunes obsolete skill dirs (after sync so the current set is known), stamps `lastSynced`, and handles branch-override auto-clear. Errors per-source are caught and logged so one bad source does not abort the whole run.
- `src/sources/github.js` — GitHub adapter. Clones/pulls the repo, then calls `resolveCheckout(source, cloneDir)` to apply any branch override (switching to a fork remote and branch). `checkoutMain` tries `main` first, falls back to `master`. When an override is active, `resolveCheckout` also fetches `origin`'s default branch via `fetchOriginMain` and rebases the override branch onto it (`git rebase origin/<mainBranch>`) before syncing — this keeps files the branch doesn't touch current with whatever's landed on main since, so only the branch's own edits stay overridden. If the rebase conflicts, it aborts (`git rebase --abort`) and falls back to syncing the branch as-is, returning `rebaseConflict: true`. Rewrites relative `node .claude/scripts/<name>` refs in command files to absolute symlinked paths via `rewriteScriptRefs`. Detects skills by checking each command file for a `name:` YAML frontmatter field; also recursively walks the entire repo via `findSkillFiles()` to find any `SKILL.md` file anywhere in the tree (supporting both flat `.claude/skills/<name>/SKILL.md` layouts and deeply nested structures like `src/plugins/<plugin>/skills/<name>/SKILL.md`). `.git`, `.processed`, and `node_modules` directories are skipped during the walk. Returns `{ commands, skills, scripts, overrideCleared, rebaseConflict }`. If `overrideCleared` is true, the branch was not found on the remote and `sync.js` will delete `source.override`.
...
- `src/symlinks.js` — Reconciles symlinks and skill dirs idempotently. Command/script links are named `<prefix>-<name>` in `~/.claude/commands/` and `~/.claude/scripts/`. Skill dirs are `~/.claude/skills/<prefix>-<skillName>/SKILL.md` written as real files (not symlinks) with the `name:` frontmatter field rewritten to the prefixed dir name so Claude Code displays the skill with the source prefix. `syncSkillDirs` writes these files; `pruneObsoleteSkillDirs(prefix, currentDirNames)` removes any `<prefix>-*` dirs whose name is not in the current sync set (called after sync); `removeSourceSkillDirs` removes all of a source's skill dirs on `remove`; `demoteCommandLinks` removes any pre-existing command symlinks for files that are now skills.
- `src/init.js` — Interactive readline wizard (invoked by the `init` command) that prompts for `JIRA_BASE_URL`, `JIRA_EMAIL`, and `JIRA_API_TOKEN`, then writes/upserts them in `~/.zshrc` or `~/.bashrc`. Finishes by calling `promptInstallGuidance()` from `agentGuidance.js`.
- `src/agentGuidance.js` — Owns the AI-agent guidance content (edit the real clone, not generated output; offer a branch/fork once edits accumulate rather than on first touch; keep an existing branch continuously pushed once it exists) and its two delivery mechanisms: `installSkill()` writes `~/.claude/skills/claude-sharester-branch-workflow/SKILL.md`, `installClaudeMdSnippet()` upserts a marker-delimited section (`<!-- claude-sharester:agent-guidance:start/end -->`) into `~/.claude/CLAUDE.md`, creating either from scratch if missing. `promptInstallGuidance()` drives the choice (skill / CLAUDE.md / both / skip) via `choose()` from `prompt.js`; used by both the `init` wizard and the standalone `agent-guidance` command. `maybeNudgeAgentGuidance()` prints a one-line reminder if neither delivery mechanism is installed, throttled to once per day via `config.lastAgentGuidanceNudge` — called from the `postAction` hook in `cli.js` alongside `checkForUpdates()`.
...
- **Skills vs commands.** If a synced markdown file has a `name:` field in its YAML frontmatter, it is a *skill* — it gets installed to `~/.claude/skills/<prefix>-<skillName>/SKILL.md` (not `~/.claude/commands/`). Adapters must return `{ commands, skills, scripts }` where `skills` is `[{ skillName, filePath }]`. Files without `name:` frontmatter are plain commands. The GitHub adapter detects skills in two ways: via `name:` frontmatter in command files, and by recursively scanning the repo tree for `SKILL.md` files (skipping `.git`, `.processed`, and `node_modules`), which supports repos with nested plugin structures. If a file was previously symlinked as a command and is now detected as a skill, `demoteCommandLinks` removes the old command symlink so the file appears only in the skills list.
- **Stale pruning fires on every sync**, not only on `remove`. `pruneStaleSymlinks(prefix)` scans `commands/` and `scripts/` for broken symlinks. `pruneObsoleteSkillDirs(prefix, currentDirNames)` removes any `<prefix>-*` skill dirs not in the current sync set — it runs *after* `syncSkillDirs` so it has the full current set to compare against.
...
- **Branch overrides** are stored as `source.override = { branch, remote? }` in `sharester.json`. On each GitHub sync, `resolveCheckout` checks whether the override branch still exists via `git ls-remote --heads`. If it is gone, `syncGithub` returns `overrideCleared: true` and `sync.js` deletes the override from config. If it still exists, the branch is rebased onto the freshest `origin/main` (or `master`) before syncing, so unrelated files reflect main's latest state rather than being frozen at whatever commit the branch diverged from; a rebase conflict falls back to the branch as-is and `sync.js` prints a warning instead of erroring.
...
```json
{
  ...
  "lastAgentGuidanceNudge": 1750000000000
}
```
